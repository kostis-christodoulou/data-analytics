...
bike <- read_csv(here::here("data", "bike99.csv")) %>%
...
model4 <- lm(bikes_hired ~ max_temp + min_temp, data = bike)
...
model5 <- lm(bikes_hired ~ max_temp + min_temp, data = bike)
...
bike$snow <- bike$snow_depth == 0

bike$snow_depth <- ifelse(is.na(bike$snow_depth), FALSE, bike$snow_depth > 0)
...
model6 <- lm(bikes_hired ~ max_temp + humidity + precipitation + snow + cloud_cover + factor(month) + wday, data = bike)
...
model5 <- lm(bikes_hired ~ max_temp + humidity + precipitation + snow + cloud_cover, data = bike)
...
# Assuming your dataframe is named 'my_dataframe' and has a date column
...
bike <- bike %>%
  ...
  mutate(
    ...
    bike_ride_lag_1day = lag(bikes_hired, n = 1),

    bike_ride_lag_7day = lag(bikes_hired, n = 7),

    max_temp_lag_1day = lag(max_temp, n = 1)
...
bike <- bike %>% 
  ...
  mutate(
    ...
    season_name = case_when(
      ...
      month_name %in%  c("Dec")  ~ 0,

      month_name %in%  c("Jan", "Feb")  ~ 1,

      month_name %in%  c("Mar", "Nov")  ~ 2,

      month_name %in%  c("Apr", "Oct")  ~ 3,

      month_name %in%  c("May", "Sep")  ~ 4,

      month_name %in%  c("Jun", "Aug", "Jul")  ~ 4,
...
model6 <- lm(bikes_hired ~ max_temp_lag_1day + precipitation + sunshine + in_lockdown + bank_holiday+ weekend + bike_ride_lag_1day + bike_ride_lag_7day + factor(year), data = bike)
...
model6 <- lm(bikes_hired ~ max_temp_lag_1day + precipitation + sunshine + in_lockdown + bank_holiday+ weekend + bike_ride_lag_1day + bike_ride_lag_7day + year, data = bike)
...
my_best_model <- broom::augment(model6) %>%
