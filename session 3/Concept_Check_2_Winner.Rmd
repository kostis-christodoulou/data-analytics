...
Here are summary statistics of salary by gender.
...
## Correlation- Scatteplot Diagram
...
`lm( Y ~ X, data= dataframe)` allows for a regression analysis. It is best to assign this to an object, e.g., `model1 <- lm(Y~X, data= .)`.  When you carry out a regression analysis, in order to see the regression output, type `msummary(model1)`. Finally, `confint(model1)` gives us confidence intervals for the estimates.
...
# no colinarity, but gender is not significant, so remove and rerun
